...
const CreateNote = (props: RouteComponentProps) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (file && file.size > MaxFileSize) {
      setErrorMsg(`File can't be bigger than ${MaxFileSize / 1000000} MB`);
      return;
    }

    setIsLoading(true);

    const createNoteURL = `./notes`;
    ...
    <PageContainer header={<HomeButton />}>
      <form onSubmit={handleSubmit}>
        ...
        <Form.Group controlId="file">
          ...
          <Form.Control
            onChange={(e) => {
              // @ts-ignore Property 'files' does not exist on type 'EventTarget & HTMLSelectElement'
